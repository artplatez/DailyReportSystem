...
@NamedQueries({
	...
	@NamedQuery(
			name = JpaConst.Q_EMP_COUNT_RESISTERED_BY_CODE, //지정된사번이 이미 DB에 있는지 검색
			query = JpaConst.Q_EMP_COUNT_RESISTERED_BY_CODE_DEF),
	@NamedQuery(
			name = JpaConst.Q_EMP_GET_BY_CODE_AND_PASS, //로그인할때 사번과 패스워드 일치확인
			query= JpaConst.Q_EMP_GET_BY_CODE_AND_PASS_DEF)
})

@Getter //모든 클래스필드에대해 getter setter자동생성(Lombok)
@Setter
@NoArgsConstructor //인수가없는 컨스트럭터를 자동생성(Lombok)
@AllArgsConstructor //모든 클래스필드를 인수로가지는 컨스트럭터를 자동생성
@Entity
